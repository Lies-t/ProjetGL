 ...
 * @version 4.12
 * @see <a href=
 * "https://github.com/junit-team/junit4/wiki/Categories">Categories at JUnit
 * wiki</a>
 */
public class Categories extends Suite {
	...
	public static class CategoryFilter extends Filter {
		...
		private CategoryFilter(boolean matchAnyIncludes, Class<?>[] inclusions, boolean matchAnyExcludes,
				Class<?>[] exclusions) {
			includedAny = matchAnyIncludes;
			excludedAny = matchAnyExcludes;
		 ...
		 * <ul>
		 * <li><tt>&quot;categories [all]&quot;</tt> for all included categories
		 * and no excluded ones;
		 * <li><tt>&quot;categories [all] - [A, B]&quot;</tt> for all included
		 * categories and given excluded ones;
		...
		private boolean hasCorrectCategoryAnnotation(Description description) {
			...
			emptyChildCategories(childCategories);

			if (matchesExcludedParentCategories(childCategories))
				return false;

			return matchesIncludedParentCategories(childCategories);
		}

		private boolean emptyChildCategories(final Set<Class<?>> childCategories) {
		...
		private boolean matchesIncludedParentCategories(final Set<Class<?>> childCategories) {
			if (included.isEmpty()) {
		...
		private boolean matchesExcludedParentCategories(final Set<Class<?>> childCategories) {
			if (!excluded.isEmpty()) {
				if (excludedAny) {
		 ...
		 * child, otherwise <tt>false</tt>. If empty <tt>parentCategories</tt>,
		 * returns <tt>false</tt>.
		 */
	...
	private static Set<Class<?>> createSet(Class<?>[] classes) {
		...
		return classes.length == 1 ? Collections.<Class<?>>singleton(classes[0])
