...
public class FailOnTimeout extends Statement {
	...
	private static final int MAX_COUNT_THREADGROUP = 5;
	private static final int MAXSIZE_THREADGROUP = 100;
	private static final int MULTIPLE_THREADGROUP = 2;
	private static final int ARRAY_SIZE_THREADGROUP = 100;
    ...
    private List<Thread> getThreadsInGroup(ThreadGroup group) {
        ...
        int threadArraySize = Math.max(activeThreadCount * MULTIPLE_THREADGROUP, MAXSIZE_THREADGROUP);
        for (int loopCount = ZERO; loopCount < MAX_COUNT_THREADGROUP; loopCount++) {
            Thread[] threads = new Thread[threadArraySize];
            int enumCount = group.enumerate(threads);
            if (enumCount < threadArraySize) {
                return Arrays.asList(threads).subList(ZERO, enumCount);
            }
            // if there are too many threads to fit into the array, enumerate's result
            // is >= the array's length; therefore we can't trust that it returned all
            // the threads.  Try again.
            threadArraySize += ARRAY_SIZE_THREADGROUP;
        }
